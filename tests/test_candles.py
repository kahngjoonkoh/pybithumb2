...
def test_get_day_candles(api_client: BithumbClient, raw_api_client: BithumbClient):
    now = datetime.now()
    market: Market = Market.from_string("KRW-BTC")
    response = api_client.get_day_candles(market, to=now)
    raw_response = raw_api_client.get_day_candles(market, to=now)
...
def test_get_day_candles_with_converting_price_unit(
    ...
    raw_response = raw_api_client.get_day_candles(
        market, to=now, convertingPriceUnit=Currency("KRW")
    )
    ...
    assert str(test_item.converted_trade_price) == str(
        raw_test_item["converted_trade_price"]
    )
